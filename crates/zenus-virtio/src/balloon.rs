    ...
    pub unsafe fn new(transport: VirtioPciTransport) -> Option<Self> {
        ...
        if transport.device_status() & 8 == 0 {
            zenus_console::kerror_code!(
                ...
                "VIRTIO-BALLOON: FEATURES_OK rejected"
            );
            return None;
        }
        ...
        let inf_dp =
            paging::virt_to_phys_raw(cr3, inf_mem as *mut VirtioQueueMem as u64).unwrap_or(0);
        ...
        if qsize0 == 0 {
            zenus_console::kerror_code!(
                zenus_console::error::codes::DRV_INIT_FAILED,
                "VIRTIO-BALLOON: Inflate queue setup failed"
            );
            return None;
        }
        ...
        let def_dp =
            paging::virt_to_phys_raw(cr3, def_mem as *mut VirtioQueueMem as u64).unwrap_or(0);
        ...
        if qsize1 == 0 {
            zenus_console::kerror_code!(
                zenus_console::error::codes::DRV_INIT_FAILED,
                "VIRTIO-BALLOON: Deflate queue setup failed"
            );
            return None;
        }

        let inflate_queue =
            VirtioQueue::new(inf_mem, qsize0, 0, transport.queue_notify_addr(0), cr3);
        let deflate_queue =
            VirtioQueue::new(def_mem, qsize1, 1, transport.queue_notify_addr(1), cr3);
