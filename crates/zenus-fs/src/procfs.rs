...
pub fn register_cpuinfo(f: DataFn) {
    *CPUINFO_FN.lock() = Some(f);
}
pub fn register_meminfo(f: DataFn) {
    *MEMINFO_FN.lock() = Some(f);
}
pub fn register_uptime(f: DataFn) {
    *UPTIME_FN.lock() = Some(f);
}
pub fn register_stat(f: DataFn) {
    *STAT_FN.lock() = Some(f);
}
pub fn register_loadavg(f: DataFn) {
    *LOADAVG_FN.lock() = Some(f);
}
pub fn register_task_count(f: CountFn) {
...
impl FileSystem for ProcFs {
    ...
    fn lookup(&self, _parent_inode: u64, name: &str) -> Option<u64> {
        ...
        if let Ok(pid) = name.parse::<u64>() {
            if pid > 0 && pid <= call_count() {
                return Some(1000 + pid);
            }
        }
        None
    }
