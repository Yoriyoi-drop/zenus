...
fn read_user_cstr(ptr: u64, max_len: usize) -> Option<alloc::borrow::Cow<'static, str>> {
    ...
    core::str::from_utf8(&buf[..len]).ok().map(|s| alloc::borrow::Cow::Owned(s.into()))
}
...
fn sys_fork(_a1: u64, _a2: u64, _a3: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    unsafe { core::arch::asm!("mov {}, gs:[8]", out(reg) kernel_rsp_top, options(nostack)); }
    ...
    let child_pid = scheduler::clone_task(
        ...
        65536,
        user_rip, // entry = user RIP (instruction after SYSCALL)
        child_cr3,
        user_rsp,
        heap_brk,
    );
        ...
        match result {
            Some((child_pid, exit_code)) => {
                ...
                if status_ptr != 0 && validate_user_range(status_ptr, 4) {
                    ...
                    unsafe { *(status_ptr as *mut u32) = status; }
                }
                return child_pid;
            }
            None => {
...
fn sys_execve(path_ptr: u64, argv_ptr: u64, envp_ptr: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    for _ in &argv_strings {
        ...
        argv_pos += 8;
        str_cur2 += argv_strings[(argv_pos - ptr_array_start - 8) as usize / 8].len() as u64; // correct str_cur2
    }
    ...
    for s in &argv_strings {
        unsafe { *((argv_pos) as *mut u64) = str_cur_pos; }
        argv_pos += 8;
        str_cur_pos += s.len() as u64;
    }

    // NULL terminate argv array
    unsafe { *((argv_array_start + argc as u64 * 8) as *mut u64) = 0; }

    // Write argc
    let user_rsp = argv_array_start - 8;
    unsafe { *((user_rsp) as *mut u64) = argc as u64; }

    zenus_console::kinfo!("execve: {} argc={} entry=0x{:x} rsp=0x{:x} cr3=0x{:x}",
        path_str, argc, loaded.entry, user_rsp, new_cr3);
...
fn sys_clone(flags: u64, stack: u64, _a3: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    unsafe { core::arch::asm!("mov {}, gs:[8]", out(reg) kernel_rsp_top, options(nostack)); }
    let user_rip: u64 = unsafe { *((kernel_rsp_top - 8) as *const u64) };
    if user_rip == 0 { return -1i64 as u64; }
...
fn sys_bind(fd: u64, addr_ptr: u64, _addrlen: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    if entry.socket_id == u64::MAX { return -1i64 as u64; }
    ...
    if net_socket::bind(sock_id, port) { 0 } else { -1i64 as u64 }
}
...
fn sys_listen(fd: u64, backlog: u64, _a3: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    if net_socket::listen(sock_id, backlog as usize) { 0 } else { -1i64 as u64 }
}
...
fn sys_connect(fd: u64, addr_ptr: u64, _addrlen: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    if net_socket::connect(sock_id, 0, dst_ip, dst_port) { 0 } else { -1i64 as u64 }
}
...
fn sys_send(fd: u64, buf_ptr: u64, len: u64, _flags: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    if !validate_user_range(buf_ptr, len) { return -1i64 as u64; }
    let data = unsafe { core::slice::from_raw_parts(buf_ptr as *const u8, len as usize) };

    if net_socket::send(sock_id, data, 0) { len } else { -1i64 as u64 }
}
...
fn sys_sendto(fd: u64, buf_ptr: u64, len: u64, _flags: u64, addr_ptr: u64, _addrlen: u64) -> u64 {
    ...
    let (dst_ip, dst_port) = if addr_ptr != 0 && validate_user_range(addr_ptr, core::mem::size_of::<SockaddrIn>() as u64) {
        let sa = unsafe { &*(addr_ptr as *const SockaddrIn) };
    ...
    if net_socket::sendto(sock_id, data, 0, dst_ip, dst_port) { len } else { -1i64 as u64 }
}

fn sys_recvfrom(fd: u64, buf_ptr: u64, len: u64, _flags: u64, addr_ptr: u64, addrlen_ptr: u64) -> u64 {
    let entry = match fd::fd_get(fd) {
        Some(e) => e,
        None => return -1i64 as u64,
    };
    ...
    match net_socket::recv(sock_id, &mut buf) {
        Some(n) => {
            ...
            if addrlen_ptr != 0 && validate_user_range(addrlen_ptr, 4) {
                unsafe { *(addrlen_ptr as *mut u32) = core::mem::size_of::<SockaddrIn>() as u32; }
            }
            n as u64
        }
        None => -1i64 as u64,
...
fn sys_mkdir(path_ptr: u64, _mode: u64, _a3: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    if fd::vfs_mkdir(&path) { 0 } else { -1i64 as u64 }
}
...
fn sys_unlink(path_ptr: u64, _a2: u64, _a3: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    if fd::vfs_unlink(&path) { 0 } else { -1i64 as u64 }
}
...
fn sys_rmdir(path_ptr: u64, _a2: u64, _a3: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    if fd::vfs_rmdir(&path) { 0 } else { -1i64 as u64 }
}
...
fn sys_rename(old_ptr: u64, new_ptr: u64, _a3: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    if fd::vfs_rename(&old_path, &new_path) { 0 } else { -1i64 as u64 }
}
...
fn sys_chmod(path_ptr: u64, mode: u64, _a3: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    if fd::vfs_chmod(&path, mode as u16) { 0 } else { -1i64 as u64 }
}
...
fn sys_chown(path_ptr: u64, owner: u64, group: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    if fd::vfs_chown(&path, owner as u32, group as u32) { 0 } else { -1i64 as u64 }
}
...
fn sys_access(path_ptr: u64, _mode: u64, _a3: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    if fd::vfs_access(&path) { 0 } else { -1i64 as u64 }
}
...
fn sys_kill(pid: u64, sig: u64, _a3: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    if sig_num == 0 || sig_num >= 64 { return -1i64 as u64; }
    ...
    if task_id.is_none() { return -1i64 as u64; }
...
fn sys_rt_sigaction(sig: u64, act_ptr: u64, oldact_ptr: u64, _sigsetsize: u64, _a5: u64, _a6: u64) -> u64 {
    let sig_num = sig as usize;
    if sig_num == 0 || sig_num >= 64 { return -1i64 as u64; }
    ...
    // Save old action
    if oldact_ptr != 0 && validate_user_range(oldact_ptr, core::mem::size_of::<KernelSigAction>() as u64) {
...
fn sys_rt_sigprocmask(how: u64, set_ptr: u64, oldset_ptr: u64, _sigsetsize: u64, _a5: u64, _a6: u64) -> u64 {
    let task_id = scheduler::current_task_id();
...
fn sys_rt_sigreturn(_a1: u64, _a2: u64, _a3: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    if let Some((restored_rsp, restored_rip, restored_rflags)) = scheduler::rt_sigreturn_restore(user_rsp) {
...
fn sys_mmap(addr: u64, length: u64, prot: u64, flags: u64, _fd: u64, _offset: u64) -> u64 {
    if length == 0 { return -1i64 as u64; }
    ...
    let vma_start = if is_fixed && addr != 0 {
        ...
        let hint = addr & !0xFFF;
        scheduler::with_vma(task_id, |vma| vma.find_free(size, hint)).flatten().unwrap_or(0)
    } else {
        scheduler::with_vma(task_id, |vma| vma.find_free(size, vma.mmap_base)).flatten().unwrap_or(0)
    };
...
fn sys_mount(source_ptr: u64, target_ptr: u64, fstype_ptr: u64, flags: u64, _data_ptr: u64, _a6: u64) -> u64 {
    if !validate_user_ptr::<u8>(source_ptr) || !validate_user_ptr::<u8>(target_ptr) || !validate_user_ptr::<u8>(fstype_ptr) {
        return -1i64 as u64;
    }
...
static SHM_TABLE: zenus_sync::spinlock::SpinLock<ShmTable> = zenus_sync::spinlock::SpinLock::new(ShmTable::new());
...
impl ShmTable {
    const fn new() -> Self {
        ShmTable { segments: [ShmSegment { key: 0, size: 0, phys: 0, attached: 0, valid: false }; MAX_SHMSEG] }
    }
    fn find_or_create(&mut self, key: u64, size: u64) -> Option<usize> {
        ...
        for i in 0..MAX_SHMSEG {
            if !self.segments[i].valid {
                ...
                for _ in 0..pages {
                    if let Some(f) = zenus_mem::frame_allocator::FRAME_ALLOCATOR.lock().alloc_frame() {
                ...
                self.segments[i] = ShmSegment { key, size, phys: first, attached: 1, valid: true };
                return Some(i);
            }
        }
        None
        ...
        if idx < MAX_SHMSEG && self.segments[idx].valid {
            ...
            if self.segments[idx].attached == 0 {
                ...
                for p in 0..pages {
                    ...
                    zenus_mem::frame_allocator::FRAME_ALLOCATOR.lock()
...
fn sys_shmat(shmid: u64, _shmaddr: u64, _shmflg: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    if !seg.valid { return -1i64 as u64; }
    ...
    let vma_start = scheduler::with_vma(task_id, |vma| vma.find_free(size, 0x3000_0000_0000)).flatten().unwrap_or(0);
    if vma_start == 0 { return -1i64 as u64; }
...
static FUTEX_TABLE: zenus_sync::spinlock::SpinLock<FutexTable> = zenus_sync::spinlock::SpinLock::new(FutexTable::new());
...
impl FutexTable {
    const fn new() -> Self {
        FutexTable { buckets: [FutexBucket { addr: 0, waiters: [FutexWaiter { task_id: 0, valid: false }; 8], count: 0 }; 64] }
    }
    ...
    fn wait(&mut self, addr: u64, val: u32, task_id: u64) -> bool {
        ...
        if user_val != val { return false; }

        let idx = Self::hash(addr);
        let bucket = &mut self.buckets[idx];

        if bucket.count < 8 {
            bucket.waiters[bucket.count] = FutexWaiter { task_id, valid: true };
            bucket.count += 1;
            true
        } else {
            false
    ...
    fn wake(&mut self, addr: u64, count: u32) -> u32 {
        ...
        for read in 0..bucket.count {
            if bucket.waiters[read].valid {
                if write != read { bucket.waiters[write] = bucket.waiters[read]; }
                write += 1;
            }
        }
        bucket.count = write;
...
fn sys_futex(uaddr: u64, op: u64, val: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    if !validate_user_range(uaddr, 4) { return -1i64 as u64; }
...
fn sys_poll(fds_ptr: u64, nfds: u64, timeout_ms: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    if fds_ptr == 0 || nfds == 0 { return -1i64 as u64; }
    let total_size = core::mem::size_of::<Pollfd>() as u64 * nfds;
    if !validate_user_range(fds_ptr, total_size) { return -1i64 as u64; }

    let start = zenus_arch::interrupts::pit::get_ticks();
    let timeout_ticks = if timeout_ms == u64::MAX { u64::MAX } else { (timeout_ms + 9) / 10 };
...
fn sys_ppoll(fds_ptr: u64, nfds: u64, _ts_ptr: u64, _sigmask_ptr: u64, _sigsetsize: u64, _a6: u64) -> u64 {
...
fn sys_setsockopt(_fd: u64, _level: u64, _optname: u64, _optval_ptr: u64, _optlen: u64, _a6: u64) -> u64 {
    0
}

fn sys_getsockopt(_fd: u64, _level: u64, _optname: u64, optval_ptr: u64, optlen_ptr: u64, _a6: u64) -> u64 {
...
fn sys_select(nfds: u64, readfds_ptr: u64, _writefds_ptr: u64, _exceptfds_ptr: u64, timeout_ptr: u64, _a6: u64) -> u64 {
    if nfds == 0 { return 0; }
    if nfds > 1024 { return -1i64 as u64; }
        ...
        if readfds_ptr != 0 {
            for fd in 0..nfds.min(64) {
                ...
                if fd_set_byte & (1 << bit_idx) != 0 {
                    ...
                    if let Some(ref e) = entry {
                        if e.socket_id != u64::MAX { is_ready = true; }
                        else if fd == 0 {
                            let s = zenus_console::serial::SerialPort::new(0x3F8);
                            is_ready = s.is_data_available() || zenus_arch::keyboard::is_key_available();
                        } else { is_ready = true; }
                    }
                    if is_ready { ready += 1; }
                }
...
fn sys_pselect6(nfds: u64, readfds_ptr: u64, writefds_ptr: u64, exceptfds_ptr: u64, timeout_ptr: u64, _sigmask_ptr: u64) -> u64 {
    sys_select(nfds, readfds_ptr, writefds_ptr, exceptfds_ptr, timeout_ptr, 0)
}

// ── Filesystem extra syscalls ──

fn sys_readlinkat(_dirfd: u64, path_ptr: u64, _buf_ptr: u64, _bufsiz: u64, _a5: u64, _a6: u64) -> u64 {
...
fn sys_symlinkat(_target_ptr: u64, _linkdirfd: u64, _linkpath_ptr: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    -1i64 as u64
}

fn sys_linkat(_olddirfd: u64, _oldpath_ptr: u64, _newdirfd: u64, _newpath_ptr: u64, _flags: u64, _a6: u64) -> u64 {
    -1i64 as u64
}

fn sys_truncate(path_ptr: u64, _length: u64, _a3: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    if read_user_cstr(path_ptr, 256).is_none() { return -1i64 as u64; }
    0
}
...
fn sys_getrlimit(resource: u64, rlim_ptr: u64, _a3: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    match resource {
        7 | 9 => { rl.rlim_cur = 256; rl.rlim_max = 4096; }
        13 => { rl.rlim_cur = 0x0000_8000_0000_0000; rl.rlim_max = 0x0000_8000_0000_0000; }
        _ => { rl.rlim_cur = u64::MAX; rl.rlim_max = u64::MAX; }
    }
    0
}
...
fn sys_getcwd(buf_ptr: u64, size: u64, _a3: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    if buf_ptr == 0 || size == 0 { return -1i64 as u64; }
    let task_id = scheduler::current_task_id();
    ...
    if !validate_user_range(buf_ptr, copy_len + 1) { return -1i64 as u64; }
    unsafe {
...
fn sys_chdir(path_ptr: u64, _a2: u64, _a3: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    // Verify path exists
    if zenus_fs::vfs::open(&path).is_none() { return -1i64 as u64; }
    let task_id = scheduler::current_task_id();
...
fn sys_prctl(option: u64, _a2: u64, _a3: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    match option {
        ...
        14 => { // PR_SET_NO_NEW_PRIVS
            0
        }
        15 => { // PR_GET_NO_NEW_PRIVS
            0
        }
        23 => { // PR_SET_TIMING
            0
        }
        36 => { // PR_SET_TAGGED_ADDR_CTRL
            0
        }
        38 => { // PR_SET_IO_FLUSHER
            0
        }
        40 => { // PR_SET_MDWE
            0
        }
        42 => { // PR_SET_MEMBARRIER
            0
        }
        57 => { // PR_SET_VMA
            0
...
fn sys_faccessat(_dirfd: u64, path_ptr: u64, _mode: u64, _flags: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    if zenus_fs::vfs::open(&path).is_some() { 0 } else { -1i64 as u64 }
}

fn sys_utimensat(_dirfd: u64, _path_ptr: u64, _times_ptr: u64, _flags: u64, _a5: u64, _a6: u64) -> u64 {
    0
}

fn sys_fstatat(_dirfd: u64, path_ptr: u64, stat_ptr: u64, _flags: u64, _a5: u64, _a6: u64) -> u64 {
    if path_ptr == 0 || stat_ptr == 0 { return -1i64 as u64; }
    let path = match read_user_cstr(path_ptr, 256) {
        Some(p) => p,
        None => return -1i64 as u64,
    };
...
fn sys_fchownat(_dirfd: u64, _path_ptr: u64, _owner: u64, _group: u64, _flags: u64, _a6: u64) -> u64 {
    0
}
...
fn sys_socketpair(domain: u64, type_: u64, _protocol: u64, fds_ptr: u64, _a5: u64, _a6: u64) -> u64 {
    if domain != 1 { return -1i64 as u64; } // AF_UNIX only
    if fds_ptr == 0 || !validate_user_range(fds_ptr, 8) { return -1i64 as u64; }
...
fn sys_sched_setscheduler(_pid: u64, _policy: u64, _param_ptr: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    0
}
...
fn sys_getrusage(_who: u64, rusage_ptr: u64, _a3: u64, _a4: u64, _a5: u64, _a6: u64) -> u64 {
    ...
    ru.ru_utime = Timeval { tv_sec: sec, tv_usec: usec };
    ru.ru_stime = Timeval { tv_sec: 0, tv_usec: 0 };
    0
}
