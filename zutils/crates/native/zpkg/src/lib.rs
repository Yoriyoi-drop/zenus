    ...
    match subcmd {
        ...
        "info" => {
            let name = match args.get(2) {
                Some(n) => n,
                None => {
                    w.write_str("Usage: zpkg info <name>\r\n");
                    return;
                }
            };
        ...
        "install" | "i" => {
            let path = match args.get(2) {
                Some(p) => p,
                None => {
                    w.write_str("Usage: zpkg install <path>\r\n");
                    return;
                }
            };
            let node = match zenus_fs::vfs::open(path) {
                Some(n) => n,
                None => {
                    w.write_str("zpkg: file not found\r\n");
                    return;
                }
            };
            ...
            if size == 0 || size > 65536 {
                w.write_str("zpkg: invalid file size\r\n");
                return;
            }
            let mut buf = alloc::vec![0u8; size];
            if node.fs.read(node.inode, 0, &mut buf).is_none() {
                w.write_str("zpkg: read failed\r\n");
                return;
            }
        ...
        "remove" | "rm" => {
            let name = match args.get(2) {
                Some(n) => n,
                None => {
                    w.write_str("Usage: zpkg remove <name>\r\n");
                    return;
                }
            };
