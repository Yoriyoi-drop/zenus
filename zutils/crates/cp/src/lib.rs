...
pub fn execute<W: Writer + ?Sized>(args: &Args, w: &mut W) {
    if args.args().len() < 2 {
        w.write_str("cp: missing operand\r\n");
        return;
    }
    ...
    let dst_node = match vfs::open(dst) {
        Some(n) => n,
        None => {
            w.write_str("cp: cannot open destination\r\n");
            return;
        }
    };
